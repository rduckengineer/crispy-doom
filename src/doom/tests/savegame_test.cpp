...
TEMPLATE_TEST_CASE("Writing in a file", "[write]", int8_t, int16_t, int32_t)
{
    GIVEN("An empty file")
    {
        ...
        TestType expected =
            GENERATE(as<TestType>{}, 0, std::numeric_limits<TestType>::min(),
        ...
        WHEN("Writing in the stream")
        {
            ...
            THEN("There is no error after the write and the written value is "
                 "correct")
            {
...
TEMPLATE_TEST_CASE("Writing in a file fails", "[write]", int8_t, int16_t,
                   int32_t)
{
...
TEMPLATE_TEST_CASE("Reading in a file", "[read]", int8_t, int16_t, int32_t)
{
    GIVEN("A file")
    {
        ...
        WHEN("Reading in the stream")
        {
            ...
            THEN("There is no error after reading and the result variable has "
                 "been set with the correct value")
            {
...
TEMPLATE_TEST_CASE("Reading in a file with an error", "[read]", int8_t, int16_t,
                   int32_t)
{
    GIVEN("A Writable file")
    {
        ...
        {
            CHECK_THROWS_AS(ReadableSaveFile{file_stream},
                            std::bad_variant_access);
        }
    }
}